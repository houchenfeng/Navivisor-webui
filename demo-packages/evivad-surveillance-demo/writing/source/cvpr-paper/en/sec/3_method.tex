...
\centering
\vspace{-5pt}
\includegraphics[width=\linewidth,trim=0 150 0 160,clip]{framework-generated-v2.png}
\caption{Algorithmic framework of \evivad{}. A surveillance clip is sampled and assessed by the quality probe before entering a frozen CLIP ViT-B/16 encoder equipped with DAA. Vision--text scoring and normality-memory retrieval provide complementary evidence to EAD, while DAG controls degradation-aware routing. The model returns an anomaly score, temporal interval, evidence citation, and confidence or abstention decision.}
\label{fig:framework}
\vspace{-6pt}
\end{figure*}
...
\begin{align}
\max_k \mathrm{sim}(h_{exp}, h_{z_k}) &< \theta_{ev} \nonumber\\
\Longrightarrow\quad E &\leftarrow \varnothing.
\end{align}
...
\STATE $F \leftarrow \mathrm{VisionTower}_{\mathrm{DAA}}(X)$
...
\ELSE
    \STATE $S_{ret} \leftarrow \mathbf{0}$ \COMMENT{retrieval miss}
\ENDIF
...
\ENDIF
\STATE $\hat{S} \leftarrow \mathrm{Smooth}(S)$
\STATE $s \leftarrow \mathrm{Mean}_t(\hat{S})$
