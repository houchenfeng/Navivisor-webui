...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{qualitative-generated-v2.png}
\caption{Qualitative comparison on one test sample. \evivad{} covers the $5.4$--$9.1$\,s event interval and returns an evidence citation, while the baseline misses the event and raises a false alarm.}
...
DAG's value is understated by clean-set metrics; degradation robustness should therefore be reported as a primary evaluation dimension.

\begin{table}[!t]
\centering
\scriptsize
\caption{Degradation robustness at light / medium / heavy severity.}
...
\centering
\vspace{-4pt}
\includegraphics[width=\columnwidth]{curves.png}
\caption{Metric curves for degradation severity, module AUC, and explanation verifiability.}
\label{fig:curves}
\vspace{-6pt}
\end{figure}
...
We analyse four failure modes: quality underestimation can route clean clips incorrectly; crowded scenes can make evidence slots ambiguous; excessive abstention can reduce recall; and an empty retrieval index weakens fusion. We report clean-set degradation, crowded-subset EAR, abstention--recall trade-offs, and an explicit \texttt{retrieval-miss} flag so each failure remains observable and countable.
