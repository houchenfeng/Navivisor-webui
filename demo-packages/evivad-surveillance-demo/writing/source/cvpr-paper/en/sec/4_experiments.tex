...
\caption{Qualitative comparison with the baseline on the same test sample. From left to right, the top row shows the input, ground-truth fall interval, baseline output, and \evivad{} output with an evidence citation. The bottom row shows frame-level anomaly scores; the shaded region marks the $5.4$--$9.1$\,s event interval and the dashed line marks the $0.5$ decision threshold.}
