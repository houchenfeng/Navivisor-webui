...
const steps = [
  {
    title: '1. 写下模糊的研究领域',
    body: '研究方向多半来自导师给定的方向，或你自己感兴趣的领域，例如三维重建、语义分割、视频异常检测。把这个领域填进「研究方向」（必填）。补充条件写在「研究目标或上下文」里，例如「和大模型结合」「做遥感场景的语义分割」，帮助缩小范围。',
  },
  {
    title: '2. 看清研究态势，选出一个课题',
    body: 'AI 会生成 OpenAlex 检索式并返回第一批相关文献，据此梳理热点、竞争激烈的方向、长期常青的主题，以及相对空白。再据此给出三个交叉研究候选课题。请选一个你最想做的。此时课题应落到具体场景或创新点，例如「基于时空事件图谱的近实时交通视频异常检测」，而不是停留在「视频异常检测」这样的大方向。',
  },
  {
    title: '3. 锁定核心文献，为实验铺路',
    body: '确定课题后，AI 会继续找最相关的参考文献及可获取的全文。这些文献既是开题报告里的相关工作，也是实验方案的主要来源。进入实验模块后，还可以借助 AI 梳理常用数据集、通用评价指标，以及你准备改进的 baseline、对比方法和可能的算法创新点。',
  },
] as const;
        ...
        <DialogContent className="max-h-[85vh] overflow-y-auto border border-[#c9dbf8] bg-[#f7fbff] sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription className="text-base font-medium leading-7 text-[#315a98]">
              把一个模糊的研究想法，收成一个带场景、带创新点的可做课题。可随时关掉，之后点「查看开题教程」还能再看。
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="rounded-xl border border-[#c9dbf8] bg-white px-3.5 py-3 text-sm leading-6 text-[#526e98]">
            ...
            <p className="mt-1.5">
              本模块用 <strong className="text-[#1f4dcb]">OpenAlex</strong> 做公开试检索：它是开放的学术文献图谱，适合先摸清领域里有哪些论文。
              <strong className="text-[#1f4dcb]"> Google 学术</strong>覆盖广、好上手，但没有稳定公开 API，结果也更杂。
              <strong className="text-[#1f4dcb]"> Scopus</strong>（以及 Web of Science）是商业数据库，收录更严，常用于正式计量，一般要走学校图书馆账号。
