    ...
    void (async () => {
      try {
        ...
        if (candidates) {
          ...
          const topics = topicsArtifact ? parseCandidateTopics(JSON.parse(await fetchArtifactText(projectId, topicsArtifact.artifactId))) : [];
          ...
          if (!cancelled && papers.length) setTask({ runId: 'workspace-demo', stage: 'first-search', status: 'completed', files: [{ name: 'candidate-papers.csv', path: 'topic/candidate-papers.csv', kind: 'csv' }], errors: [], papers, counts: { papers: papers.length, previewed: papers.length, deduplicated: papers.length, returned: papers.length, requested: papers.length, targetReached: true }, candidateStatus: topics.length === 3 ? 'completed' : 'idle', candidates: topics.length === 3 ? topics : undefined, coreStatus: coreLiterature.length ? 'completed' : 'idle', coreManifest: coreLiterature.length ? { status: 'completed', files: ['core-references.csv'], counts: { references: coreLiterature.length, pdfDownloaded: 0 } } : undefined, demoCoreLiterature: coreLiterature.length ? coreLiterature : undefined, isDemo: topics.length === 3 && coreLiterature.length > 0 });
...
function parseCandidateTopics(value: unknown): NonNullable<ResearchTaskSnapshot['candidates']> {
  ...
  return raw.map((item) => {
    ...
    return { label, title: String(candidate.title), oneSentenceDefinition: String(candidate.question ?? '候选研究方向'), researchDesign: Array.isArray(candidate.methodSteps) ? candidate.methodSteps.join(' → ') : String(candidate.researchDesign ?? ''), expectedInnovation: Array.isArray(candidate.innovations) ? candidate.innovations.join('、') : String(candidate.expectedInnovation ?? ''), rationale: Array.isArray(candidate.evidencePaperIds) ? `依据文献：${candidate.evidencePaperIds.join('、')}` : String(candidate.rationale ?? '') };
