...
export function TopicPage() {
  ...
  const loadDemo = () => { window.sessionStorage.removeItem(activeRunKey); setInterest(demoInterest); setContext(demoContext); setTask(demoTaskSnapshot); setSelectedCandidate(null); setError(''); setPage(1); };
  ...
  return <main className="min-h-0 flex-1 overflow-auto bg-[linear-gradient(108deg,#f5f6f5_0%,#e5f0fd_51%,#bfdcff_100%)] text-[#19386f]">
    <div className="mx-auto min-h-full w-full max-w-[1500px] px-5 py-7 sm:px-8 lg:px-12">
      ...
      {page === 1 && <><div className="mt-5 flex justify-end"><LoadWorkspaceDemoButton compact /></div><DirectionPage interest={interest} setInterest={setInterest} context={context} setContext={setContext} task={task} error={error} isRunning={isRunning} isComplete={isComplete} onRun={runSearch} onCancel={cancelSearch} onRetry={runSearch} onNext={() => setPage(2)} onLoadDemo={loadDemo} /></>}
