    ...
    void (async () => {
      try {
        ...
        if (candidates) {
          ...
          if (!cancelled && papers.length) setTask({ runId: 'workspace-candidate-papers', stage: 'first-search', status: 'completed', files: [{ name: 'candidate-papers.csv', path: 'topic/candidate-papers.csv', kind: 'csv' }], errors: [], papers, counts: { papers: papers.length, previewed: papers.length, deduplicated: papers.length, returned: papers.length, requested: papers.length, targetReached: true } });
        }
      } catch (requestError) {
...
function DemoCoreLiteraturePage({ onBack }: { onBack: () => void }) { return <section className="mt-7 rounded-[28px] bg-white p-6 shadow-[0_15px_40px_rgba(42,83,143,0.14)] sm:p-10"><div className="flex items-start gap-4"><div className="grid size-12 shrink-0 place-items-center rounded-2xl bg-[#e7f0ff] text-[#1f4dcb]"><FlaskConical className="size-6" /></div><div><p className="text-xs font-black tracking-[0.16em] text-[#5f85b8] uppercase">Step 03 · 教学演示</p><h2 className="mt-2 text-2xl font-black tracking-[-0.03em] text-[#183b70]">核心文献智能分析</h2><p className="mt-3 max-w-2xl text-sm font-semibold leading-6 text-[#617da9]">这是基于所选候选课题的模拟交接页，用来展示后续实验、写作和投稿模块需要消费什么证据。</p></div></div><div className="mt-7 rounded-2xl border border-[#f0d6a5] bg-[#fff8e9] p-4 text-xs font-bold leading-5 text-[#8b641e]">教学演示数据 · 文献来自 OpenAlex 元数据快照。</div><div className="mt-6 space-y-3">{demoTaskSnapshot.demoCoreLiterature?.map((paper) => <article key={paper.openalexId} className="rounded-2xl border border-[#d8e5f6] bg-[#fbfdff] p-4"><div className="flex flex-wrap items-start justify-between gap-3"><h3 className="min-w-0 flex-1 text-sm font-black leading-6 text-[#244a7d]">{paper.title}</h3><a href={paper.openalexId} target="_blank" rel="noreferrer" className="text-xs font-black text-[#1f4dcb] underline">OpenAlex 来源</a></div><p className="mt-2 text-xs font-semibold leading-5 text-[#526e98]">{paper.whyRelevant}</p></article>)}</div><div className="mt-7 rounded-2xl bg-[#eef5ff] p-4 text-xs font-semibold leading-5 text-[#55749f]"><strong className="text-[#315a98]">交接说明：</strong>后续实验模块消费“已确认课题 + 核心文献依据”；写作模块消费实验方案和结果；投稿模块只消费论文草稿与实验记录。Demo 不会自动替代这些真实产物。</div><div className="mt-7 flex flex-wrap items-center justify-between gap-3 border-t border-[#e3ecf8] pt-5"><Button variant="outline" onClick={onBack} className="rounded-xl border-[#9bbce8] font-black text-[#1f4dcb]"><ArrowLeft />返回候选课题</Button><GoToExperimentButton /></div></section>; }
