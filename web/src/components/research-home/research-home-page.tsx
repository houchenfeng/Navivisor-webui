...
import { ArrowRight, ClipboardList, FlaskConical, PenLine, Send } from 'lucide-react';
        ...
        <div className="research-route mt-10 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {modules.map(({ title, copy, icon: Icon, to }, index) => (
            ...
            >
              ...
              <span className="mt-6 inline-flex items-center gap-1 text-sm font-bold text-[#1F4DCB]">
                进入模块 <ArrowRight className="size-4 transition-transform group-hover:translate-x-1" />
              </span>
            </button>
          ))}
        </div>
        <p className="mt-7 text-center text-xs font-medium text-[#7890b6]">
          {t('Select or create a thread to start chatting.')}
        </p>
      </section>
    </main>
  );
