...
function ResultsPage({ go }: { go: (step: ExperimentStep) => void }) {
  ...
  return <div className="flex flex-col gap-5">
    ...
    <section className="overflow-x-auto rounded-2xl bg-white/90 p-5"><h3 className="font-semibold">主实验结果 · 与其他算法对比</h3>{!fromWorkspace ? <p className="mt-1 text-xs text-muted-foreground">mean±std，3 seeds；↑ 越高越好，延迟越低越好</p> : null}<table className="mt-3 w-full min-w-[760px] text-sm"><thead><tr className="border-b text-left">{comparisonHeaders.map((head) => <th key={head} className="p-2">{head}</th>)}</tr></thead><tbody>{comparisonRows.map((row, rowIndex) => <tr key={`${row[0]}-${rowIndex}`} className="border-b last:border-0">{row.map((cell,index) => <td key={`${rowIndex}-${index}`} className={cn('p-2', index === 0 && 'whitespace-nowrap')}>{cell}</td>)}</tr>)}</tbody></table></section>
    <section className="overflow-x-auto rounded-2xl bg-white/90 p-5"><h3 className="font-semibold">模块消融实验</h3>{!fromWorkspace ? <p className="mt-1 text-xs text-muted-foreground">DAA：低秩领域适配；EAD：证据锚定解码；DAG：退化感知门控</p> : null}<table className="mt-3 w-full min-w-[680px] text-sm"><thead><tr className="border-b">{ablationHeaders.map((head) => <th key={head} className="p-2">{head}</th>)}</tr></thead><tbody>{ablationRows.map((row,index) => <tr key={index} className="border-b text-center last:border-0">{row.map((cell, cellIndex) => <td key={`${index}-${cellIndex}`} className={cn('p-2', index === ablationRows.length - 1 && 'font-semibold text-[#1F4DCB]')}>{cell}</td>)}</tr>)}</tbody></table></section>
    {hydration.robustnessRows.length > 0 ? <section className="overflow-x-auto rounded-2xl bg-white/90 p-5"><h3 className="font-semibold">退化鲁棒性专项结果</h3><table className="mt-3 w-full min-w-[760px] text-sm"><thead><tr className="border-b text-left">{hydration.robustnessHeaders.map((head) => <th key={head} className="p-2">{head}</th>)}</tr></thead><tbody>{hydration.robustnessRows.map((row, rowIndex) => <tr key={`${row[0]}-${rowIndex}`} className="border-b last:border-0">{row.map((cell, cellIndex) => <td key={`${rowIndex}-${cellIndex}`} className={cn('p-2', rowIndex >= hydration.robustnessRows.length - 2 && 'font-semibold', rowIndex === hydration.robustnessRows.length - 1 && cellIndex === 2 && 'text-[#1F4DCB]')}>{cell}</td>)}</tr>)}</tbody></table></section> : null}
